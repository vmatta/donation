...
public class DesignationService {
  ...
  /**
   * Receives list of designations. First extract designations with unique orderId and delete from database if present. Then it groups designations based on orderId and update the
   * orderIdNumber and stores in the database.
   *
  ...
  public List<Designation> saveDesignation(List<Designation> designations) {
    ...
    for (Map.Entry<String, List<Designation>> listEntry : groupedBasedOnOrderId) {
      ...
      designationList.stream().forEach(designation -> updatedWithOrderItemNumber
          .add(new Designation(designation.getOrderId(), atomicInteger.getAndIncrement(), designation.getDesignationName(), designation.getDesignationAmount())));
    }
